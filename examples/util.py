...
def load_key_text(filepath=''):
    ...
    if not path.isfile(filepath):
        raise ValueError('Path does not point to a file', filepath)
...
def get_client_id():
    ...
    return environ.get('CLIENT_ID', 'ZmFrZS1hcHAtdXVpZDpvcGVyYXRvci1k')


def get_uuid_env_var(name):
    """Get an environment variable or UUID."""
    value = environ.get(name)
...
def get_raw_request_object(overrides=None):
    ...
    request_object = {
        ...
        'login_hint': environ.get('LOGIN_HINT', 'MSISDN:411223344556'),
        # From the spec:
    ...
    correlation_id = get_uuid_env_var('correlation_id')
